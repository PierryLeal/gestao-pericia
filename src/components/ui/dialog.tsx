  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          // `[&>*]:min-w-0` matters: DialogContent is a grid container, and a grid
          // item's default `min-width: auto` refuses to shrink below its content's
          // min-content size — for any descendant with `white-space: nowrap` (e.g. a
          // `truncate`d combobox value), that min-content is the FULL untruncated
          // text width, which silently forces the dialog's child open past the
          // modal's edge instead of letting it truncate. Overriding it here fixes
          // that for every dialog in the app, not just the one that first hit it.
